...
public class EditBookActivity extends AppCompatActivity {
    ...
    private EditText editTitleEditAuthor;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        editTitleEditAuthor = findViewById(R.id.edit_book_author);

        final Button button = findViewById(R.id.button_save);
        button.setOnClickListener(view -> {
            Intent replyIntent = new Intent();
            if (TextUtils.isEmpty(editTitleEditText.getText()) || TextUtils.isEmpty(editTitleEditAuthor.getText())){
                setResult(RESULT_CANCELED, replyIntent);
            }else{
                String title = editTitleEditText.getText().toString();
                replyIntent.putExtra(EXTRA_EDIT_BOOK_TITLE, title);
                String author = editTitleEditAuthor.getText().toString();
